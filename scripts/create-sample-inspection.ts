...
const INSPECTIONS_DIR = "inspections";

async function createSampleInspection() {
  try {
    ...
    const exists = inspections.some((i: any) => i.id === sampleInspection.id);
    if (!exists) {
      inspections.push(sampleInspection);
      await AsyncStorage.setItem(INSPECTIONS_KEY, JSON.stringify(inspections));
      console.log("✅ Vistoria fictícia criada com sucesso!");
    } else {
      console.log("ℹ️ Vistoria fictícia já existe");
    }
  } catch (error) {
    console.error("❌ Erro ao criar vistoria fictícia:", error);
  }
}
